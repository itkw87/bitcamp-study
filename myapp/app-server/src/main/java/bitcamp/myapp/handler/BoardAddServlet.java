...
public class BoardAddServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      // 각각의 파트에서 값을 꺼낸다.
      // 멀티파트에서는 filter에서 설정한 캐릭터 인코딩이 통하지 않는다.
      // 각각의 파트를 꺼낼 때 설정해줘야 한다.
      ...
      // 웹 애플리케이션 환경정보에서 /upload/board 디렉토리의 실제 경로를 물어 본다.
      ...
      for (FileItem part : parts) {
        if (part.isFormField()) { // 일반 데이터
          ...
          part.write(new File(uploadDir, filename)); // (베이스경로, 아래경로(파일이름))
