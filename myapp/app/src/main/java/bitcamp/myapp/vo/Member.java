...
public class Member {
  ...
  // 왜? Object의 equals()는 단순히 인스턴스의 주소가 같은지 비교하기 때문이다.
  ...
  public boolean equals(Object obj) {
    ...
    if (this.getClass() != obj.getClass()) { // 파라미터로 넘겨받은 인스턴스가 Member타입이 아니라면 비교할 필요조차 없음
      return false;
    }

    // 위 조건에서 this가 가리키는 인스턴스의 클래스와
    // 파라미터 obj가 가리키는 인스턴스의 클래스가 같다고
    // 결론이 났기 때문에 다믐과 같이 obj를 Member 타입으로 형변환한다.
    Member m = (Member) obj;
    ...
    // return false;
    // }
    // if (this.getGender() != m.getGender()) { // 숫자라 바로 비교가능
    // return false;
    // }
    return true;
  }
